  ...
  return (
    <div className="relative bg-card border border-border rounded-xl px-4 py-3.5 overflow-hidden">
      ...
      <div
        ...
        style={{ color: VALUE_COLORS[accent] }}
      >
        {value}
      </div>
...
const getValorProposta = (n: Negociacao) => {
  try {
    const salvo = DB.get<any>(`proposta_${n.id}`);
    if (salvo && salvo.length > 0) {
      const secoes = salvo[0].secoes || [];
      return secoes.reduce((total: number, s: any) => {
        const produto = s.itens?.reduce((acc: number, i: any) => acc + (i.qtd * i.valorUnd), 0) || 0;
        return total + produto + (s.servicoValor || 0);
      }, 0);
    }
  } catch {}
  return n.valor || 0;
    ...
    <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 mb-4">
        ...
        <KPI
          ...
          value={mesFechadas}
          sub={mesFechadas > 0 ? '✓ Bom desempenho' : 'Nenhuma ainda'}
          accent="green"
        />
        <KPI
          label="Leads a validar"
          value={leadsNew}
          sub="Aguardando qualificação"
          accent={leadsNew > 0 ? 'red' : 'blue'}
        />
        <KPI
          label="Ticket médio"
          value={`R$ ${fmt(ticketMed)}`}
          sub={`${fechadas.length} vendas fechadas`}
          accent="purple"
        />
        <KPI
          ...
          value={`${convGeral}%`}
          sub={`Mês: ${convMes}%`}
          accent={convGeral >= 50 ? 'teal' : 'amber'}
        />
      </div>
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-[12px]">
            <thead>
              <tr>
                {['Código', 'Cliente', 'Título', 'Valor', 'Status'].map(h => (
                  <th
                    key={h}
                    className="bg-ast-bg3 text-ast-text3 text-[9px] tracking-[0.6px] px-4 py-2.5 text-left font-medium uppercase border-b border-border"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {recent.map(n => (
                <tr
                  key={n.id}
                  className="hover:bg-ast-bg3 cursor-pointer transition-colors"
                  onClick={() => onNavigate('negociacoes')}
                >
                  <td className="px-4 py-2.5 border-b border-border font-semibold text-foreground text-[11px]">
                    {n.codigo}
                  </td>
		  ...
		  <td className="px-4 py-2.5 border-b border-border font-semibold text-primary">
		    R$ {fmt(getValorProposta(n))}
              ...
              {!recent.length && (
                <tr>
                  <td colSpan={5} className="text-center text-ast-text3 py-10 text-[12px]">
