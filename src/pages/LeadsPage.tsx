...
import type { Lead, Empresa, Parceiro, Cliente, Negociacao, Recorrente } from '@/lib/types';
...
export default function LeadsPage() {
  ...
  const users = DB.get<any>('users').filter((u: any) => u.ativo !== false);

  let leads = DB.get<Lead>('leads');
  if (filter !== 'todos') leads = leads.filter(l => l.status === filter);
  ...
  const confirmConvert = () => {
    ...
    const resp = users.find((u: any) => u.id === convertForm.responsavelId);
    ...
    if (convertForm.tipoProjeto === 'negociacao') {
      ...
      const neg: Negociacao = {
        ...
        clienteTel: clienteExistente.tel,
        ...
        historico: [{
          ...
          acao: 'Criação',
          detalhes: `Convertido do lead — Origem: ${convertingLead.origem}`,
        }],
        indicacaoNome: convertingLead.indicacaoNome || '',
        indicacaoId: convertingLead.indicacaoId || '',
      };
      ...
      const rec: Recorrente = {
        id: uid(),
        codigo: 'REC-' + String(DB.get<Recorrente>('recorrentes').length + 1).padStart(3, '0'),
        data: new Date().toISOString().split('T')[0],
        clienteId: clienteExistente.id,
        clienteNome: clienteExistente.nome,
      ...
      >
        <FormSection title="Dados do lead">
          <FormGrid>
            ...
            {/* Campo livre se o parceiro não estiver cadastrado */}
            {!form.indicacaoId && (
        ...
        {convertingLead?.indicacaoNome && (
          <div className="mt-4 p-3 bg-ast-bg3 border border-border rounded-lg">
            ...
            <div className="text-xs text-ast-text3 mt-1">
              5% sobre produtos · 10% sobre serviços
            </div>
          </div>
        )}
      </Modal>
