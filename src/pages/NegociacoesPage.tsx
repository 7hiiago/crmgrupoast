...
export default function NegociacoesPage({
  ...
  const deleteNeg = (id: string) => {
    ...
    // Remove proposta salva junto
    localStorage.removeItem(`ast_proposta_${id}`);
    refresh();
  };
                  ...
                  <div className="flex gap-1" onClick={e => e.stopPropagation()}>
                    ...
                    {onNavigate && (
                      <Btn
                        variant="icon"
                        sm
                        onClick={() => onNavigate('proposta', n.id)}
                        title="Gerar proposta visual"
                      >
                        🖊
                      </Btn>
                    )}
                    {isAdmin && (
                      <Btn variant="icon" sm onClick={() => deleteNeg(n.id)}>×</Btn>
                    )}
                  </div>
                </Td>
              </tr>
